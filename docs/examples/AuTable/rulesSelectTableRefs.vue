<template>
  <au-layout-page>
    <au-layout-page-item>
      <au-table
        title="单元格编辑AuSelectTable--获取单个Ref实例"
        :table="table"
        :columns="table.columns"
        :isShowPagination="false"
        ref="singleEditAuSelectTable"
...
// const singleEditAuSelectTable: any = (ref < HTMLElement) | (null > null)
const singleEditAuSelectTable: any = ref(null)
// 添加数据条数
const num = ref(3)
// 下拉表格数据
...
// 保存
const save = () => {
  // console.log('另一种获取的table--ref', singleEditAuSelectTable.value)
  singleEditAuSelectTable.value.saveMethod(data => {
...
// 清除校验规则
const clearValidate = () => {
  singleEditAuSelectTable.value.clearValidate()
}
// 重置表单
const reset = () => {
  singleEditAuSelectTable.value.resetFields()
}
// 重置用户名称下拉表格数据
const resetSelect = () => {
  console.log("重置用户名称下拉表格数据")
  singleEditAuSelectTable.value.resetTselectTable()
}
const editDel = (item, index, row) => {
  // console.log('9966--', singleEditAuSelectTable.value.save())
  row.splice(index, 1)
