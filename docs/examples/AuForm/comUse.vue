<template>
  <au-layout-page>
    <au-layout-page-item>
      <au-form ref="AuFormDemo" v-model="formOpts.ref" :formOpts="formOpts" :widthSize="1" />
    </au-layout-page-item>
  </au-layout-page>
</template>
...
// 获取ref
const AuFormDemo: any = ref<HTMLElement | null>(null)
...
//   const { valid, formData } = await AuFormDemo.value.validate()
...
// 重置form表单
const resetForm = () => {
  AuFormDemo.value.selfResetFields()
}
...
const formOpts = reactive<FormTypes.FormOpts>({
  ...
  fieldList: [
    ...
    },
    {
      label: "下拉选择表格-单选",
      value: "deptCode",
      placeholder: "au-select-table单选使用",
      ref: "selectTableRef",
      comp: "au-select-table",
      isSelfCom: true,
      bind: {
        isKeyup: true,
    ...
    },
    {
      ...
      placeholder: "au-select-table多选使用",
      comp: "au-select-table",
      isSelfCom: true,
      bind: {
        multiple: true,
