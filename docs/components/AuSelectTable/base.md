...
:::demo `isShowQuery` 是否开启查询条件配置，其配置继承查询条件组件（AuQueryCondition 组件）的所有属性/插槽/事件；注意：`不可设置AuQueryCondition组件的maxVisibleSpans属性` ;
...
:::demo 在`au-select-table`标签中添加`tableLoading`，loading 文字设置`loadingText`
...
| isShowQuery                 | 是否允许配置查询条件(继承 AuQueryCondition 的所有属性、事件、插槽)                      | Boolean                             | false        |
...
| multipleFixed               | table 是否固定多选                                                                      | Boolean                             | true         |
| radioFixed                  | table 是否固定单选                                                                      | Boolean                             | true         |
| useVirtual                  | table 是否开启虚拟滚动                                                                  | Boolean                             | false        |
| virtualShowSize             | 虚拟列表的渲染行数                                                                      | Number                              | 30           |
| multipleDisableDelete       | 多选--之前选中的数据不在新数据源下，是否隐藏`tag`删除 icon                              | Boolean                             | true         |
| defaultValIsOpenRadioChange | 默认赋值--是否开启单选事件                                                              | Boolean                             | false        |
| radioSameIsCancel           | 单选选中同一条数据，是否取消选中                                                        | Boolean                             | true         |
| border                      | 下拉表格是否显示边框                                                                    | Boolean                             | true         |
| tableSize                   | 下拉表格大小可选值：`large`、`default`、`small`                                         | String                              | ''           |
| radioSelectValLabel         | 单选回显 label 需要动态配置 defaultSelectVal 才有选中效果                               | String                              | -            |
| isRadioEchoLabel            | 单选--是否开启回显 label                                                                | Boolean                             | true         |
| remote                      | 是否开启远程搜索                                                                        | Boolean                             | false        |
| remoteMethod                | 自定义远程搜索方法                                                                      | Function                            | 搜索输入的值 |
| tableLoading                | table loading                                                                           | Boolean                             | false        |
| loadingText                 | table loading 文字                                                                      | String                              | '加载中...'  |
