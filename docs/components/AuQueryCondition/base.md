...
:::demo 1、通过`getRefs`事件获取`opts`中某项设置的`ref`；2、重置方式(`调用handleEvent`)是：handleEventChangeRef.value.handleEvent({isChange:true, val: "" }, "workshopNum")`；3、通过`AuQueryCondition`的ref获取`queryState`在把你需要重置的值赋值为`null`。
AuQueryCondition/getRefs
:::

### AuQueryCondition 参数配置
...
| -----arrLabel     | el-select/el-checkbox-group/el-radio-group 组件 options label 显示 | string           | label      |
| -----arrKey       | el-select/el-checkbox-group/el-radio-group 组件 options key 显示   | string           | key        |
...
| ref         | 可以通过 getRefs 获取对应的 ref                                                  | String           | 当前项的 ref 标识          |
...
| getRefs<el-tag>v1.5.1</el-tag> | 获取 AuQueryCondition 使用组件的 ref               | opts 中 最好需要传 ref 标识来区分是哪个组件的 ref |
...
| handleEvent | 动态修改查询条件值     | Function<au-tip content='(flag: boolean, {val: 需要修改的值}, dataIndex: string) => void'/> |
