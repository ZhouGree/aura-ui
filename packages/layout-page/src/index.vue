...
export interface AuLayoutPageProps {
...
const props = withDefaults(defineProps<AuLayoutPageProps>(), {
...
const AuLayoutPageRef = ref(null)
...
const backToTop = () => {
  scrollTop.value = 0
  ;(AuLayoutPageRef.value as any).scrollTop = 0
}
onMounted(() => {
  const pageItems = (AuLayoutPageRef.value as any).querySelectorAll(".au_layout_page_item")
...
onActivated(() => {
  if (!props.keepScrollDisabled) {
    ;(AuLayoutPageRef.value as any).scrollTop = scrollTop
  }
})
</script>
