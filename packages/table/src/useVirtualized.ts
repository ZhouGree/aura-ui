...
export function useVirtualized() {
  ...
  // 获取dom元素
  const getDom = () => {
    actualHeightContainerEl.value = document.querySelector(".au_table_use_virtual .el-scrollbar__view")
    translateContainerEl.value = document.querySelector(".au_table_use_virtual .el-table__body")
    scrollContainerEl.value = document.querySelector(".au_table_use_virtual .el-scrollbar__wrap")
  }
  ...
  const updateRenderedItemCache = (index: number) => {
    ...
    nextTick(() => {
      // 获取所有列表项元素
      const Items: HTMLElement[] = Array.from(document.querySelectorAll(".au_table_use_virtual .el-table__row"))
      // 进行缓存
      Items.forEach(el => {
