<template>
  <el-form
    id="au_query_condition"
    v-bind="$attrs"
    :label-width="labelWidth"
    :form="queryState.form"
    size="default"
    class="au-query-condition"
    :style="{
  ...
  >
    ...
    >
      ...
      <template v-if="opt.isSelfCom">
        <component
          :is="opt.comp"
          :ref="opt.comp === 'au-select-table' ? (el: any) => handleRef(el,opt, i) : (el: any) => getRefs(el, opt, i)"
...
const props = withDefaults(defineProps<AuQueryConditionProps>(), {
  opts: () => ({}),
  labelWidth: "120px",
  btnCheckBind: () => ({}),
  btnResetBind: () => ({}),
