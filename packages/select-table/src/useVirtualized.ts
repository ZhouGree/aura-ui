...
export function useVirtualized() {
  ...
  const getDom = (props: { multiple: boolean }) => {
    ...
    if (props.multiple) {
      getElements(".au_select_table_multiple");
    } else {
      getElements(".au_select_table_radio");
    }
  ...
  const updateRenderedItemCache = (index: number) => {
    ...
    nextTick(() => {
      // 获取所有列表项元素
      const Items: HTMLElement[] = Array.from(document.querySelectorAll(`${isMultiple.value ? `.au_select_table_multiple` : `.au_select_table_radio`} .el-table__row`))
      // 进行缓存
      Items.forEach(el => {
